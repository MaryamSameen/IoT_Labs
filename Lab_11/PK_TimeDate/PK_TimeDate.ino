...
String getPakistanDate() {
  timeClient.update();
  unsigned long epochTime = timeClient.getEpochTime();
  
  struct tm *ptm = gmtime((time_t *)&epochTime);
  int monthDay = ptm->tm_mday;
  int currentMonth = ptm->tm_mon + 1; // Month is 0-11
  int currentYear = ptm->tm_year + 1900;
  
  // Format as "22-08-2025"
  ...
  if (millis() - lastReadTime >= SENSOR_DELAY) {
    ...
    if (readDHT(&temp, &hum)) {
      if (millis() - lastSendTime >= SEND_INTERVAL) {
        ...
        String pakistanDate = getPakistanDate();
        Serial.println("Current Pakistan Date: " + pakistanDate);
        Serial.println("Current Pakistan Time: " + pakistanTime);
        sendToFirebase(temp, hum, pakistanDate, pakistanTime);
        lastSendTime = millis();
      }
    }
    lastReadTime = millis();
...
void sendToFirebase(float temp, float humidity, String pakistanDate, String pakistanTime) {
  if (WiFi.status() != WL_CONNECTED) {
